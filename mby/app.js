...
const SCENES = {
  intro: () => {
    ...
    const text = "Velkommen til 🌙🏘️ Måneby! Du er en 🤠 eventyrer der er rejst til Måneby. Du vågner op i et mærkeligt rum 🛏️. Du må ud, og gerne få så mange point som muligt. God fornøjelse! 😀";
    const actions = [{label: "begynd", handler: () => goto("rollHp")}];
    return Scene({ text, actions });
  },
  rollHp: () => {
    const text = "Først skal du finde ud af hvor mange ❤️ HP du skal have. Slå med 🎲 terningen. Læg 7 til resultatet og så mange HP har du.";
  ...
  chooseSkills: () => {
    const text = "Undervejs i eventyret kan du få forskellige ⚔️ våben. Der er fire slags våben: 🗡️ sværd, 🏹 bue, 🔪 kniv og 🪾 pind. Jo flere point du har, jo bedre er du til at ramme med det våben. Hvilken type eventyrer du vil være?"
    const actions = [
  ...
  scene1Kiste: () => {
    const text = "I kisten finder du en 🗝️ nøgle med nummer 11 på.";
  ...
  scene4: () => {
    PLAYER.hp.val += 4;
    const text = "Du sover på 🏠 kroen. Du spiser et 🥗 måltid og får +4 ❤️ HP. Bagefter går du videre.";
    const actions = [{label: "⬆️ fortsæt", handler: () => goto("scene23")}];
    return Scene({ text, actions });
  },
  scene5: () => {
    const text = "Du kommer til et ❌ kryds langs skoven. Der er en sti ind i skoven og en større vej. Hvilken vej tager du?";
    const actions = [
  ...
  scene6: () => {
    ...
    PLAYER.gold.val += 10;
    const text = "Du vinder over manden. Du slår ham til jorden og han taber sine ting. Du tager hans 🔪 kniv og 10 🪙 guldmønter.";
    const actions = [{label: "⬆️ fortsæt", handler: () => goto("scene21")}];
    return Scene({ text, actions });
  },
  scene7: () => {
    let text = "Du går videre ud i byen. Der sidder en 👴 mand ved vejen, og du vælger at spørge ham: »Hvordan kommer jeg ud af byen?« Han svarer at for 1 🪙 guldmønt kan han vise dig vej ud til den store port.";
    let actions = [{label: "🏃 gå videre", handler: () => goto("scene3")}];
    if (PLAYER.gold.val < 1) {
      text += " Du har ikke penge nok. Du må gå videre.";
      return Scene({ text, actions });
    }
  ...
  scene8: () => {
    let text = "Du fortsætter og kommer til en lille 🏠 kro. Et 🥗 måltid og 🛏️ overnatning koster 3 🪙 guld.";
    let actions = [
      {label: "🛏️ bliv på kroen", handler: () => { PLAYER.gold.val -= 3; goto("scene4") }},
      {label: "🏃 gå videre", handler: () => goto("scene23")}
    ];
    if (PLAYER.gold.val < 3) {
      text += " Du har ikke penge nok og må gå videre.";
      actions.shift();
    } else {
  ...
  scene9: () => {
    ...
    if (PLAYER.inventory.val.includes(ITEMS.key11)) {
      text += " Du har en nøgle med nummer 11 på. Vil du ind i skuret?";
      actions.unshift({label: "🚪 gå ind", handler: () => goto("scene16")});
    } else {
      text += " Du har desværre ikke nogen nøgle der passer.";
    }
    return Scene({ text, actions });
  },
  scene10: () => {
  ...
  scene16: () => {
    ...
    const text = "Inde i skuret er der en bænk. På bænken finder du et 🗡️ sølvsværd.";
  ...
  scene21: () => {
    let text = "Du kommer forbi en lille bod. Du kan købe en 🏹 bue (5 angreb) for 10 guld eller en 🦯 stav (2 angreb) for 3 guld.";
    let actions = [{label: "🏃 gå videre", handler: () => goto("scene7")}];
    if (PLAYER.gold.val < 3) {
      text += " Du har ikke råd til at købe noget.";
      return Scene({ text, actions });
    }
  ...
  scene22: () => {
    const text = "Du kommer ud af skoven og ind i byen. Der er to veje, en stor og en lille. Hvilken vej tager du?";
    const actions = [
  ...
  scene26: () => {
    ...
    ENEMY.hp = enemy.hp;
    const text = `Endelig! Du når porten. Nu skal du kæmpe mod en vagt. Han har ${enemy.hp} HP og et spyd der skader ${enemy.attackDamage}. Vælg et angreb:`;
  ...
  scene27: () => {
    let text = "Du går ud af døren. Du står i en have. Der er en lille robåd. Der er også en lille sø.";
    ...
    if (PLAYER.inventory.val.includes(ITEMS.fishrod)) {
      ...
      text += " Du har desværre ikke nogen fiskestang.";
    }
    return Scene({ text, actions });
  },
  scene28: () => {
  ...
  scene29: () => {
    const text = `Du har tabt kampen. Manden tager dig og putter dig i fængsel. Du har tabt! Du fik ${PLAYER.points.val} point.`;
    const actions = [{label: "⏮️ til start", handler: () => goto("intro")}];
    return Scene({ text, actions });
  },
  scene30: () => {
...
function battle(battleScene, winScene, lossScene, enemy) {
  ...
  if (playerHits) {
    ...
    text += ` Dit angreb lykkedes. Du skader fjenden ${PLAYER.attackDamage}.`;
    if (ENEMY.hp <= 0) {
  ...
  text += ` Fjenden har ${ENEMY.hp} HP tilbage og går til modangreb.`;
